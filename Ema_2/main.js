...
mainScene.createPaddle = function() {
 ...
 this.paddle.setDisplaySize(110,20);
...
mainScene.createBlocks = function() {
   ...
   for(var i = 0; i < 6; i++ ){
       ...
       for(var j = 0; j < 8; j++ ){
           ...
           var block = this.blocks.create(80 + j * 80, 80 + i * 40, color);
           block.setOrigin(0,0);
           block.setDisplaySize(80, 40);
           block.refreshBody();
       }
   }
...
mainScene.failToHit =  function () {
    ...
    if(this.life <= 0) {
        // 0.5秒後にゲームオーバー
            this.Text = this.add.text(350, 300, 'Game Over:' , {
        font: '80px Open Sans',
        fill: '#ff0000'
    });
